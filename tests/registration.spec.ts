...
// To verify registration page elements are visible
    ...
    test('Should display error message for invalid email and password', async ({ page }) => {
        ...
        await registrationPage.postalCodeField.fill('50450');
        await registrationPage.houseNumberField.fill('123');
        await registrationPage.streetField.fill('Jalan Bukit Bintang');
        await registrationPage.cityField.fill('Kuala Lumpur');
        await registrationPage.stateField.fill('Wilayah Persekutuan');
        await registrationPage.phoneField.fill('1234567890');
